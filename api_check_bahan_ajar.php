...
if ($code !== '6a12a09069248191a7d76d45beb44c50') {
  http_response_code(403);
  echo json_encode(['ok' => false, 'error' => 'forbidden'], JSON_UNESCAPED_UNICODE);
  exit;
}

function has_column_api_check(mysqli $db, string $table, string $column): bool {
...
function stmt_bind_params_api_check(mysqli_stmt $stmt, string $types, array $values): void {
  $bind = [];
  $bind[] = $types;
...
if ($uid > 0) {
  ...
  try {
    if ($hasNoHpCol && !$isEmail && count($candPhones) > 0) {
      ...
      $stmt = $mysqli->prepare("SELECT id, username, password, role, ($exprAccess) AS access_bahan_ajar FROM users WHERE username = ? OR ($phoneExpr IN ($ph)) LIMIT 1");
    } else {
      $stmt = $mysqli->prepare("SELECT id, username, password, role, ($exprAccess) AS access_bahan_ajar FROM users WHERE username = ? LIMIT 1");
    }
  } catch (mysqli_sql_exception $e) {
    $stmt = null;
  }
  ...
  if ($hasNoHpCol && !$isEmail && count($candPhones) > 0) {
    ...
    stmt_bind_params_api_check($stmt, $types, $values);
  } else {
...
$access = (int)($row['access_bahan_ajar'] ?? 1);
